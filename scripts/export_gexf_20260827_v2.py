# -*- coding: utf-8 -*-
"""GEXF 导出 + 渲染验证 v2（2026-08-27，阿语合并后重建 19:01 的图库）
1. 从 Neo4j 拉全图（32435 节点/114845 边）→ networkx DiGraph → GEXF
2. read_gexf 读回 → 节点/边数对账（渲染验证第一步：可解析）
3. matplotlib 渲染 Top 主题共现子图 PNG + 节点类型分布 PNG
输出: E:\大挑\产出\重建_20260826\knowledge_graph_20260827_v2.gexf + *_v2.png + gexf_验证报告_20260827_v2.md
"""
...
GEXF = os.path.join(OUT, "knowledge_graph_20260827_v2.gexf")
PNG_SUB = os.path.join(OUT, "gexf_topics_subgraph_20260827_v2.png")
PNG_TYPES = os.path.join(OUT, "gexf_node_types_20260827_v2.png")

EXPECT_NODES = 32435   # 合并后实测: Paper12233+Topic1168+Domain26+Year84+Author14612+Venue4312
EXPECT_EDGES = 114845   # 合并后实测
...
plt.title("GEXF v2 节点类型分布 (合并后 20260827)")
plt.ylabel("节点数")
...
plt.title("Top25 主题共现子图 v2 (GEXF 渲染验证, 合并后)")
plt.axis("off")
plt.tight_layout()
...
# 验证报告
report = f"""# GEXF 导出渲染验证报告 v2（合并后，2026-08-27）
...
"""
with open(os.path.join(OUT, "gexf_验证报告_20260827_v2.md"), "w", encoding="utf-8") as f:
    f.write(report)
print("验证报告已写")
