...
# 5. 写 JSON（v2 独立文件，保留 v1 可对照）
snap = {
    "meta": {
        "file": "graph_snapshot_20260821_v2.json",
        "generated_at": "2026-08-21",
        "generated_by": "build_snapshot_v2.py（复刻 rebuild_neo4j.py 聚合口径；计数取 Neo4j 实测）",
        "source": "本地 Neo4j 重建图库（2026-08-21 晚：B1主表追加56篇后重建）",
        "purpose": "给 Manus 页面/连 GitHub 读图用；仅聚合层",
        "change_note": "v2 = v1 + 新增56篇阿拉伯文旅文献（8.17新增550 PDF 中未进主表部分补齐）",
        "rules": [
            "不含全文 PDF/正文文本",
...
p_json = os.path.join(OUT, "graph_snapshot_20260821_v2.json")
with open(p_json, "w", encoding="utf-8") as f:
    json.dump(snap, f, ensure_ascii=False, indent=1)
print("写入:", p_json, os.path.getsize(p_json), "bytes")

p_csv = os.path.join(OUT, "topic_year_heat_v2.csv")
