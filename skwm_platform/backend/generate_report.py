...
lines.append(f"| 动力学预测 AUC | 未复现（XGBoost） |")
lines.append(f"")
...
lines.append(f"基于 {DATA.n_snapshots} 年时间序列（{ah}–{hi}）的 XGBoost 模型（AUC=未复现），预测未来 5 年趋势：")
lines.append(f"")
...
lines.append(f"*数据基于 {DATA.n_snapshots} 年时间切片 × {total_state:,} 条状态向量 × XGBoost(AUC=未复现)*")
