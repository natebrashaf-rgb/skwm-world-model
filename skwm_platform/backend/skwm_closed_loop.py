...
class ClosedLoopEvaluator:
    ...
    def hit_rate(self, ctrl: SKWMClosedLoopController,
                 eval_years: list[int], user: str,
                 L: int = 4, M: int = 5, B: int = 8, k: int = 10) -> float:
        """命中率 = 推荐主题在 L 年后真正进入 Top-k 的比例"""
        hits = 0
        for t in eval_years:
            ...
            hits += len(recommended & top_real) / max(1, len(recommended))
        return hits / len(eval_years)
