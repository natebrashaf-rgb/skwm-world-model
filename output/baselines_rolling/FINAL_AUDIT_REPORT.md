...
结论：10（本轮 8–12）个 FAIL **全部 = 训练/采样环节的跨进程随机性（单次快照差），非口径错、非泄漏、非数据版本差**。c0c1 是 8/28 单次快照，本脚本每轮也是单次快照，两个任意单次快照间训练型模型的差异即噪声。证据链：
- 噪声量化（audit_tmp/noise_quant.py）：同进程 5 轮 xgboost/rssm **逐位一致**（排除 seed/代码 bug）；跨进程漂移：xgboost h1 MAE ∈ [20.3737, 20.6603]、rssm h1 MAE ∈ [71.0251, 71.0433]；c0c1 值（20.3987 / 71.0111）**全部落在漂移范围内**。
- rssm 采样随机性代码证据：WorldModel.imagine 默认 deterministic=False（随机采样潜变量 z），M2RSSM.predict 未传 deterministic=True。
- M0 27 格全等已经把数据/评测年/主题集/补 0/聚合口径全部锁定。

| FAIL | 模型 | 指标 | 原因 | 处理 |
|---|---|---|---|---|
| h1 xgboost MAE diff 0.03–0.21 | xgboost | MAE | 跨进程训练浮点噪声（范围含 c0c1 值） | 不改数字，报告范围；同进程可复现 |
...
| h1 rssm MAE diff 0.025–0.032 | rssm | MAE | stochastic imagine 跨进程采样差 | 不改数字；若要逐位复现 rssm 需锁 deterministic，但会改变全部 rssm 数字，违背「不改数字」原则 |
| h1 rssm P@10 diff 0.07–0.10 | rssm | P@10 | 随机采样排名跳档 | 同上 |
| h3 rssm Spearman diff 0.012–0.016 | rssm | Spearman | 随机采样 | 同上 |
| h3 rssm P@10 diff 0.17–0.33 | rssm | P@10 | 随机采样（rssm 预测精度差→排名近随机，P@10 档位波动大） | 同上 |
| h5 rssm P@10 diff 0.03–0.27 | rssm | P@10 | 同上 | 同上 |

若 c0c1 侧要求逐格一致：需把对账目标改为「口径一致（M0 全等）+ 训练模型取三轮范围/中位数」，或对 c0c1 同法多次跑取中位数。**未采用**：为对齐截图/旧快照硬改数字（违反审计最高原则）。
...
无硬缺口。可选增强：把 94.7MB split_manifest.csv 推 GitHub（需组长确认可接受仓库体积 +~95MB）；或组员 Brashaf 侧需要跑「origin 训练」验证时用本包命令即可。
