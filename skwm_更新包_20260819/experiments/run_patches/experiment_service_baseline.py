...
class S0StaticService:
    ...
    def answer(self, question: dict) -> dict:
        ...
        if question["ground_truth_type"] == "emerging_topics":
            ...
            # 证据量 = 有正增速的主题数(各题不同 → 置信度有区分度,防盲评猜组)
            n_ev = sum(1 for n, v in sv.items() if v[1] > 0)
            answer = {
                "qid": qid,
                "level": "S0",
            ...
            answer = {
                ...
                "confidence": self._calibrate_confidence(n_ev, n_max=12000),
                "has_prediction": False,
                "has_uncertainty": False,
                "has_risk_warning": False,
            }
            ...
            answer = {
                ...
                "evidence": "基于文献统计",
                "confidence": self._calibrate_confidence(n_ev, n_max=14000),
                "has_prediction": False,
                "has_uncertainty": False,
            ...
            topics = sorted(sv.items(), key=lambda x: -x[1][0])[:10]
            n_ev = sum(1 for n, v in sv.items() if v[0] > 0)
            answer = {
                "qid": qid,
                "level": "S0",
                "topics": [{"name": n, "heat": v[0]} for n, v in topics],
                "evidence": "当前热度排序外推，无预测模型",
                "confidence": self._calibrate_confidence(n_ev, n_max=4000),
                "has_prediction": False,
                "has_uncertainty": False,
                "has_risk_warning": False,
            }
            ...
            answer = {
                ...
                "confidence": self._calibrate_confidence(n_ev, n_max=6000),
                "has_prediction": False,
                "has_uncertainty": False,
                "has_risk_warning": False,
            }
...
class S1TrendService:
    ...
    def answer(self, question: dict) -> dict:
        ...
        if question["ground_truth_type"] in ("emerging_topics", "forecast"):
            forecast_results = []
            r2s = []
            # 按热度取 Top-50(原代码 list(sv.keys())[:50] 是字典插入序,垃圾词混入)
            top_names = [n for n, v in sorted(sv.items(), key=lambda x: -x[1][0])[:50]]
            ...
            # 新增: 推荐题型也给出 Top-5 趋势外推,避免三组答案雷同
            top_names = [n for n, v in sorted(sv.items(), key=lambda x: -x[1][0])[:5]]
            outlook, r2s = [], []
            for name in top_names:
                fc = self._linear_forecast(name, 3)
                ...
                outlook.append({
                    ...
                    "predicted_heat_3y": round(fc["preds"][-1], 1) if fc["preds"] else current,
                    "r2": fc["r2"],
                })
                r2s.append(fc["r2"])
            base["trend_outlook"] = outlook
            base["has_prediction"] = True
            base["has_uncertainty"] = False
            base["evidence"] = "S0基线 + 线性趋势外推"
            base["confidence"] = self._conf_from_r2(
            ...
            # 新增: 覆盖度题型给出稀疏子方向趋势
            sparse_names = [n for n, v in sorted(sv.items(), key=lambda x: x[1][0])[:10]]
            outlook, r2s = [], []
            for name in sparse_names:
                ...
                if fc["preds"]:
                    outlook.append({
                        "name": name,
                        "trend": round(fc["preds"][-1] - sv[name][0], 1),
                        "r2": fc["r2"],
                    })
                    r2s.append(fc["r2"])
            base["growth_outlook"] = outlook
            base["has_prediction"] = True
            base["has_uncertainty"] = False
            base["evidence"] = "S0基线 + 线性趋势外推"
            base["confidence"] = self._conf_from_r2(
            ...
            top_names = [n for n, v in sorted(sv.items(), key=lambda x: -x[1][0])[:50]]
            base["confidence"] = self._conf_from_r2(
                float(np.mean([fc["r2"] for name in top_names
...
class S2RSSMService:
    ...
    def _load_model(self):
        """v3: 优先加载 model_rssm_v3.pt (delta目标/无泄漏训练),
        兼容旧 model_rssm.pt。meta.target 决定推理路径。"""
        self.model_meta = {}
        for name in ("model_rssm_v3.pt", "model_rssm.pt"):
            ...
            try:
                ...
                data = torch.load(str(model_path), map_location="cpu",
                                  weights_only=False)
                c = WMConfig(**data.get("config", {}))
                m = WorldModel(c)
                m.load_state_dict(data["model"])
                m.eval()
                self.model = m
                self.model_meta = data.get("meta", {}) if isinstance(data, dict) else {}
                print(f"  [S2] 已加载 {name} (target={self.model_meta.get('target', 'level-旧版')})")
                return
            except Exception:
                continue
        self.model = None

    def _rssm_forecast(self, topic: str, horizon: int, B: int = 10) -> dict:
        """v3修复: ① 严格按 as_of_year 截断历史 ② delta模型+历史窗口推理
        ③ std 保留在 log 空间 (供 _heat_range 与置信度公式使用)"""
        ...
        for y in years:
            ...
            levels.append(np.array([
                np.log1p(max(0.0, float(v[0]))), float(v[1]),
                np.log1p(max(0.0, float(v[2]))), np.log1p(max(0.0, float(v[3])))
            ], dtype=np.float32))
        L = np.stack(levels)                     # [Y,4] 编码水平
        D = np.diff(L, axis=0)                   # [Y-1,4] delta
        hist = torch.tensor(D[-8:][None], dtype=torch.float32)   # [1,T,4]
            ...
            try:
                with torch.no_grad():
                    pd = self.model.imagine_from_history(
                        hist, a_future, deterministic=False)
                pe = L[-1][None, :] + np.cumsum(pd[0].numpy(), axis=0)  # [h,4]
                all_heat.append(np.maximum(0, np.expm1(pe[:, 0])))
                all_log.append(pe[:, 0])
            except Exception:
                break
        ...
        sh = np.stack(all_heat)                  # [B,h] 热度空间
        sl = np.stack(all_log)                   # [B,h] log空间
        # v3.1: 用中位数聚合, 防止个别采样rollout爆炸拉爆均值 (如 nation +17258)
        preds = [float(np.median(sh[:, t])) for t in range(horizon)]
        stds = [float(sl[:, t].std()) for t in range(horizon)]
        return {"preds": preds, "std": stds}

    @staticmethod
    def _heat_range(fc: dict, t: int) -> list:
        """log 空间 std → 热度空间 95% 区间 [low, high] (RSSM 不确定性带)"""
        if not fc["preds"] or not fc["std"]:
            return [0.0, 0.0]
        p = max(1e-6, fc["preds"][t])
        s = max(0.0, fc["std"][t])
        lp = np.log1p(p)
        low = float(np.expm1(max(0.0, lp - 1.96 * s)))
        high = float(np.expm1(lp + 1.96 * s))
    ...
    def answer(self, question: dict) -> dict:
        ...
        if question["ground_truth_type"] in ("emerging_topics", "forecast"):
            ...
            # 新增: 推荐题型也给出 Top-5 RSSM 外推 + 不确定性,避免三组答案雷同
            top_names = [n for n, v in sorted(sv.items(), key=lambda x: -x[1][0])[:5]]
            outlook, stds = [], []
            for name in top_names:
                fc = self._rssm_forecast(name, 3)
                ...
                outlook.append({
                    ...
                    "current_heat": current,
                    "predicted_heat_3y": round(fc["preds"][-1], 1) if fc["preds"] else current,
                    "uncertainty": round(fc["std"][-1], 3) if fc["std"] else 0.0,
                    "range": self._heat_range(fc, -1),
                })
                if fc["std"]:
                    stds.append(fc["std"][-1])
            base["rssm_outlook"] = outlook
            base["has_prediction"] = True
            base["has_uncertainty"] = True
            base["evidence"] = "S0基线 + RSSM多步预测 + 不确定性估计"
            base["confidence"] = round(
            ...
            stds = []
            top_names_rg = [n for n, v in sorted(sv.items(), key=lambda x: -x[1][0])[:50]]
            for name in top_names_rg:
                fc = self._rssm_forecast(name, 3)
                if fc["preds"]:
                    stds.append(fc["std"][-1])
            base["confidence"] = round(
        ...
        SUPPORTED = ("emerging_topics", "forecast", "report_generation",
                     "recommendation", "coverage_assessment")
        if question.get("ground_truth_type") not in SUPPORTED:
