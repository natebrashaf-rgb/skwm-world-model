...
class WMConfig:
    ...
    stoch_std: float = 1.0  # 消融开关: =0 时先验/后验退化为确定性潜状态 (无随机)
...
class RSSM(nn.Module):
    ...
    def _dist(self, params: torch.Tensor) -> torch.distributions.Normal:
        ...
        stoch_std=0 时标准差归零 → 确定性潜状态 (消融 B);
        此时关闭 validate_args (Normal 默认要求 scale>0)"""
        mean, std = params.chunk(2, -1)
        std = (F.softplus(std) + 0.1) * self.c.stoch_std
        return torch.distributions.Normal(
            mean, std, validate_args=(self.c.stoch_std > 0))
    ...
    def step(self, h: torch.Tensor, z: torch.Tensor, a: torch.Tensor,
        ...
        Args:
            ...
            sample: False 时用分布均值 (确定性推理, 结果可复现)
        Returns:
...
class WorldModel(nn.Module):
    ...
    def loss(self, x_seq: torch.Tensor, a_seq: torch.Tensor
        ...
        # 消融 B (stoch_std=0): 分布退化为确定性, KL=0/0=NaN → 跳过 KL 项
        if self.c.stoch_std > 0:
            ...
            L_dyn = torch.zeros((), device=x_seq.device)
            L_rep = torch.zeros((), device=x_seq.device)
    ...
    @torch.no_grad()
    def imagine_from_history(self, x_hist: torch.Tensor,
                             a_future: torch.Tensor,
                             deterministic: bool = False) -> torch.Tensor:
        """v3: 先沿历史窗口走后验建立潜状态, 再用先验 rollout 未来

        修复旧 imagine() 只用单帧 x0、丢弃历史窗口的问题:
        GRU 需要先消化 T 步历史才有时间上下文。

        Args:
            x_hist: [B, T, x_dim] 历史观测窗口 (与训练同分布的编码)
            a_future: [B, L, a_dim] 未来干预 (全零 = 无干预时间预测)
            deterministic: True 时用先验均值 (点估计可复现),
                           False 时先验采样 (配多次 rollout 得不确定性)
        Returns:
            [B, L, x_dim] 未来 L 步预测
        """
        B, T = x_hist.shape[0], x_hist.shape[1]
        h, z = self.rssm.initial(B, x_hist.device)
        zeros_a = torch.zeros(B, self.c.a_dim, device=x_hist.device)

        # 1) 消化历史 (observe 模式: 用后验)
        #    deterministic=True 时后验也用均值 — 修复原实现 observe 阶段无条件
        #    post.rsample() 导致 deterministic 推理仍不可复现的问题
        for t in range(T):
            e = self.enc(x_hist[:, t])
            h = self.rssm.cell(torch.cat([z, zeros_a], -1), h)
            post = self.rssm._dist(self.rssm.post(torch.cat([h, e], -1)))
            z = post.mean if deterministic else post.rsample()

        # 2) 先验 rollout 未来 (imagine 模式)
        preds = []
        for t in range(a_future.shape[1]):
            h = self.rssm.cell(torch.cat([z, a_future[:, t]], -1), h)
            prior = self.rssm._dist(self.rssm.prior(h))
