...
EVIDENCE_NEUTRAL = {
    ...
    "RSSM趋势预测": "趋势预测",   # 防兜底正则产生"趋势趋势预测"病句
}
...
def anonymize_answer(answer: dict, qid: str) -> dict:
    ...
    summary = {}
    # 预测类内容优先（S1/S2 的差异核心）
    # 注: rssm_outlook 键名含敏感词,摘要中统一改名为中性键 forecast_outlook
    for key in ["forecast", "prediction", "uncertainty", "prediction_note",
                "trend_outlook", "rssm_outlook", "growth_outlook",
                "sections", "evidence"]:
        if key in public:
            out_key = "forecast_outlook" if key == "rssm_outlook" else key
            summary[out_key] = public[key]
    # 再放基础内容
...
def generate_blind_package(as_of_year: int = 2023):
    ...
    # 字数均衡化: 同一题三组的"回答摘要"补齐到等长(防盲评按篇幅猜组别)
    conds = {}
    for condition in ["S0", "S1", "S2"]:
        conds[condition] = json.loads(
            (BASE / f"condition_{condition}" / "blind_answers.json")
            .read_text(encoding="utf-8"))
    n_q = min(len(v) for v in conds.values())
    for i in range(n_q):
        max_len = max(len(conds[c][i]["回答摘要"]) for c in conds)
        for c in conds:
            cur = conds[c][i]["回答摘要"]
            if len(cur) < max_len:
                conds[c][i]["回答摘要"] = cur + " " * (max_len - len(cur))
    for condition in ["S0", "S1", "S2"]:
        out_path = BASE / f"condition_{condition}" / "blind_answers.json"
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(conds[condition], f, ensure_ascii=False, indent=2)
    print("  [均衡] 三组回答摘要已补齐到等长")
