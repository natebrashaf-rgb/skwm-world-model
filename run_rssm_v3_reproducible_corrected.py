...
def load_data():
    ...
    # Pre-register 100 topics using only information available by the training cutoff.
    pre_years=[int(y) for y in sv if y != '_wm' and int(y) <= 2019]
    candidate=set().union(*(set(sv[str(y)]) for y in pre_years))
    ranked=sorted(candidate,key=lambda t:(-float(sv.get('2019',{}).get(t,[0,0,0,0])[0]),str(t)))
    kwm.topics=ranked[:100]; kwm.topic_names={t:t for t in kwm.topics}
    return kwm
...
def evaluate(model,kwm,config,seed):
    years=list(range(1995,2025)); train_cut=2019; rows=[]; audit=[]; candidate_topic_count=len(kwm.topics)
    for h in (1,3,5):
        ...
        for origin in origins:
            ...
            m.update({'model':config['name'],'seed':seed,'horizon':h,'forecast_origin':origin,'history_start':origin-config['T']+1,'history_end':origin,'target_year':origin+h,'train_cutoff':train_cut,'topic_count':len(kwm.topics),'topic_selection':'top 100 by 2019 heat among topics observed by 2019; no post-2019 information','candidate_topic_count':candidate_topic_count,'mae':float(np.mean(np.abs(pred-actual))),'rmse':float(np.sqrt(np.mean((pred-actual)**2)))})
